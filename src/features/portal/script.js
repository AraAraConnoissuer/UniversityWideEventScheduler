...
function renderPublicDayDialog() {
  ...
  $('publicDayEvents').innerHTML = items.map(({ event, occurrence }) => {
    ...
    return `<article class="public-day-event" data-public-event-id="${escapeHtml(event.id)}" role="button" tabindex="0" style="border-left-color:${escapeHtml(organizationColor(state.store, event))};--event-accent-color:${escapeHtml(eventAccentColor(state.store, event))}"><strong>${escapeHtml(event.title)}</strong><p>${escapeHtml(formatTime(occurrence.start_time))} to ${escapeHtml(formatTime(occurrence.end_time))}</p><p>${escapeHtml(scheduleOwnerLabel(event))} - ${escapeHtml(event.venue)}</p><p>${escapeHtml(category.name)} - ${escapeHtml(cap(event.event_status))}</p><p>${escapeHtml(event.public_description)}</p></article>`;
